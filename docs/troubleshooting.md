...
|---|---|---|
| `vec_init_then_push` | `07_collections.rs` | Allowed on `fn main()` — the exercise's point is demonstrating `Vec::new()` + `.push()`. |
...
| `unnecessary_fold` | `12_closures_iterators.rs` | Allowed on the `let` statement — the exercise's point is `fold` as the general reduce tool, not the shortest way to a product. |
| `useless_vec` | `12_closures_iterators.rs` | **Fixed** — switched `vec![1, 2, 3, 4, 5, 6]` to an array literal `[1, 2, 3, 4, 5, 6]`; no pedagogical loss since the exercise only iterates, never mutates. |
